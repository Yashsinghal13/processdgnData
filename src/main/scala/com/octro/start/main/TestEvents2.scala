...
object TestEvents2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      val df = savedfilePaths.toDF("path","datetime","hashed_path")
      // Select the last two fields (timestamp and md5)
      val selectedDF = df.select("hashed_path","datetime")
      selectedDF.write.mode(SaveMode.Overwrite).parquet("/tmp/simran/processed_paths")
