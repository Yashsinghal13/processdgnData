...
object DGN_Events_Main {
  ...
  def main(args: Array[String]): Unit = {
    OPrinter.logAndPrint("*************** Welcome to DGN Connector On 8th Mar'24  ***************")
    ...
    //    val hourPaths = List[String]("17","19", "20", "21", "22", "23")
...
//        val startDate = dateFormat.parseDateTime("2024/01/25")
//        val endDate = dateFormat.parseDateTime("2024/03/08")
...
//      val currentDate = startDate.plusDays(day).toString(dateFormat)
      val dayPathToRead = s"/dgn/ovs/raw_events/ovs/events/prod/$currentDate"
...
//            }
      ...
      case e: Exception => {
        ...
        Notifier.sendEMail("dgn_ovs@octro.com", s"Error occurred for date $current_date()" + e.getMessage, "simran.maurya@octrotalk.com", "Action required : Couldn't process dgn-ovs data")
    ...
    if (hourPathToRead.size != 0) {
      try {
        hourPathToRead.foreach(hour => {
          ...
          val schemaJson = spark.read.text("/tmp/simran/schema.txt").toDF("col")
          ...
          retRDD.map(x => x._1).saveAsTextFile(s"/dgn/ovs/kafka_events/$start_date/$hour")
        ...
        case e: Exception => {
          ...
          Notifier.sendEMail("dgn_ovs@octro.com", s"Error occurred in dgn_ovs connector for date $start_date" + e.getMessage, "simran.maurya@octrotalk.com", "Action required : Couldn't process dgn-ovs data")
          OPrinter.logAndPrint(e.getMessage)
        }
      }
    }
  ...
  def writeLastReadPathOnHDFS(hour: String, path: String, sparkSession: SparkSession): Unit = {
    ...
    hourDf.write.mode(SaveMode.Overwrite).text(s"${path}/dgn_ovs_lastReadHour21Feb.txt")
  }
  ...
  def readLastPathOnHDFS(path: String, sparkSession: SparkSession) = {
    ...
    val hourDf = sparkSession.read.text(s"${path}/dgn_ovs_lastReadHour21Feb.txt").collect().head.getString(0)
    hourDf
  }
  ...
  def getGOCIDfromUSERID(userId: String): String = {
    ...
    val guestId = userId
    val appid = "20240221mrpi" // OVS app_id
