...
object DGN_OVS_Events {
  val dgnOvsTopic = "dgn_ovs_events" //"DGN-Events"
  //  val dgn_map_userid_gocid_tbl = "dgn_ovs_numid_gocid"
  val dgn_map_gocid_userid_tbl = "ovs_gocid_uuid"
  val dgn_map_hashedid_gocid_tbl = "ovs_uuid_gocid"

  def main(args: Array[String]): Unit = {
    OPrinter.logAndPrint("*************** Welcome to DGN Connector On 14th Jun'24  ***************")
    ...
    val eventNames = List("transaction", "level_up", "install", "start_session", "worlds", "spin", "facebook_connect", "end_session")

    val transnCol = List("purchase_count", "price", "session_id", "event_id", "platform_id","country_code")
    val levelUpCol = List("session_id", "level", "vip_level", "event_id", "platform_id")
    val installCol = List("session_id", "event_id", "platform_id")
    val startSesCol = List("version", "balance", "bingo_balls_balance", "platform_id", "blackdiamond_elite_active", "session_id", "event_id", "level", "vip_level", "device_type", "country_code")
    val worldsCol = List("session_id", "last_unlocked_world", "event_id", "platform_id")
    val spinCol = List("bet_amount", "event_id", "session_id", "platform_id","level", "slot_id")
    val facebookConnectCol = List("session_id", "fb_id", "event_id")
val endSesCol = List("event_id", "session_id", "platform_id", "balance", "level", "vip_level", "country_code")
    var mapperEventCol: Map[String, List[String]] = Map("transaction" -> transnCol, "level_up" -> levelUpCol, "install" -> installCol, "start_session" -> startSesCol, "worlds" -> worldsCol, "spin" -> spinCol, "facebook_connect" -> facebookConnectCol, "end_session" -> endSesCol)
    ...
    try {
      ...
      case e: Exception => {
        ...
        Notifier.sendEMail("dgn_ovs@octro.com", s"Error occurred " + e.getMessage, "shreyansh.maurya@octrotalk.com,govind.gopal@octrotalk.com,akashdeep.singh@octrotalk.com", "Action required : Couldn't process dgn-ovs data")
...
//          val repRDD= jsonData.repartition(numPartitions)
          ...
          var retRDD = jsonData.mapPartitions(partition => {
            ...
            val newRDD = partition.map(row => {
              if (row != null && row.contains("event_type") && row.contains("ts") && row.contains("user_id") ) {
                ...
                try {
                  if (hashed_user_id == "") {
                    Notifier.sendEMail("dgn_ovs@octro.com", s"Data packet doesn't contain user_id for \n : $row", "shreyansh.maurya@octrotalk.com,govind.gopal@octrotalk.com,akashdeep.singh@octrotalk.com", "Action required : Couldn't process dgn-ovs data")
                  }
                  ...
                  case e: Exception => {
                    OPrinter.logAndPrint(s"Unable to fetch gocid for user $hashed_user_id from hbase")
                  }
                }
                ...
                else if (gocid_from_hbase == "" && hashed_user_id != "") {

                  accum.add(1)

                  OPrinter.logAndPrint(s"Fetching gocid for $hashed_user_id")
          ...
          non_emptyRDD.map(x => x._1).saveAsTextFile(s"/dgn/ovs/kafka_events_new/$lastReadDateHour")
          ...
          if(accum.value > 0) {
            Notifier.sendEMail("dgn_ovs@octro.com", s"API hits count for $hour: ${accum.value} ", "shreyansh.maurya@octrotalk.com,govind.gopal@octrotalk.com,akashdeep.singh@octrotalk.com", "API hits for DGN OVS")
          }
        })
        ...
        case e: Exception => {
          OPrinter.logAndPrint("Spark Context Stopped")
          spark.sparkContext.stop()
          Notifier.sendEMail("dgn_ovs@octro.com", s"Error occurred in dgn_ovs connector " + e.getMessage, "shreyansh.maurya@octrotalk.com,govind.gopal@octrotalk.com,akashdeep.singh@octrotalk.com", "Action required : Couldn't process dgn-ovs data")
          OPrinter.logAndPrint(e.getMessage)
        }
      }
    }
  ...
  def getPathsToRead(lastHourRead: String, currDay: DateTime, nextDay: String, spark: SparkSession): List[String] = {
    ...
    if (lastHourRead.substring(lastHourRead.length - 2).toInt >=20) {
      if(dirExists(s"/dgn/ovs/raw_events/ovs/events/prod/$nextDay"))
        {
      val currentPlusOneList = hdfs.listStatus(new Path(s"/dgn/ovs/raw_events/ovs/events/prod/$nextDay"))
      currentPlusOneList.foreach(x => {
        //   println(x.getPath)
    ...
    if (fileListAfterSubstring.size < 2) {
      Notifier.sendEMail("dgn_ovs@octro.com", s"There must be some lag on DGN-OVS data copying on HDFS ", "shreyansh.maurya@octrotalk.com,govind.gopal@octrotalk.com,akashdeep.singh@octrotalk.com", "Action required : Check for lag in DGN_OVS paths on hdfs")
      OPrinter.logAndPrint("Spark Context Stopped")
      spark.sparkContext.stop()
    }
