...
object DGN_Events {
  ...
  def main(args: Array[String]): Unit = {
    OPrinter.logAndPrint("*************** Welcome to DGN-LTS Connector On 31st Jul'24  ***************")
    ...
    try {
      ...
      val df = savedfilePaths.toDF("path", "timestamp", "hashed_path")
      // Select the last two fields (timestamp and md5)
//      val selectedDF = df.select("hashed_path","timestamp")
      df.write.mode(SaveMode.Append).parquet("/tmp/simran/processed_paths")
...
/*
      ...
      val resultList = allfilePaths.filterNot { case (_, _, hash) => list1Hashes.contains(hash) }

      // Print the result
      resultList.take(5).foreach(println)
      println(s"Count of resultList: ${resultList.size}")

      val status = hourwiseDataReader(resultList, spark, eventNames, mapperEventCol, numPartitions)
  ...
  def hourwiseDataReader(hourPathToRead: ListBuffer[(String,String,String)], spark: SparkSession, eventNames: List[String], mapperEventCol: Map[String, List[String]], numPartitions: Int): String = {
    import spark.implicits._
    ...
    if (hourPathToRead.size != 0) {
      try {
        ...
        hourPathToRead.foreach(hour => {
          ...
          OPrinter.logAndPrint(s"Reading data for path $path")
          val jsonRDD = spark.sparkContext.textFile(s"$path")
          ...
          val lastReadDateHour= path.split("prod/")(1).slice(0, 10)
          //          val timestamp = getCurrentTimestamp

          OPrinter.logAndPrint(s"Writing data on hdfs for $path at $lastReadDateHour $getCurrentTimestamp")
          ...
          import spark.implicits._
          val df_ = spark.createDataFrame(Seq((hour._3,hour._2))).toDF("hashed_path","datetime")          // df.show(10,false)
